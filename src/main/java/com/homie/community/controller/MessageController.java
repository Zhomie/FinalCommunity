...
import com.homie.community.service.MessageService;
...
public class MessageController implements CommunityConstant {
    ...
    public String getLetterList(Model model, Page page){
        ...
        if(conversationList!=null){
            for(Message message:conversationList){
                ...
                int targeId = user.getId() == message.getFromId()? message.getToId() : message.getFromId();
                map.put("target",userService.findUserById(targeId));
    ...
    /*请求是异步的，要加上@ResponseBody*/
    @ResponseBody
    public String sendLetter(String toName,String content){
        //Integer.valueOf("abc");
        User target = userService.findUerByName(toName);
        if(target==null){
            return CommunityUtil.getJSONString(1,"目标用户存在");
        }
